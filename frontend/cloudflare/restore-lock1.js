...
(()=>{
 ...
 function renderOutlook(){
  ...
  mount.innerHTML=outlook?`<p class="ctdPublishedNote">Published ${e(dateLabel(outlook.generated_at))} · ${ordered.length} players · Current outlook, not start/sit recommendations</p><div class="ctdOutlookGrid">${ordered.map(p=>`<article class="categoryCard"><h3>${e(p.position)} · ${e(p.team)}</h3><h2><button class="ctdProfileLink" data-profile="${e(p.player)}">${e(p.player)} →</button></h2><p class="copy">${e(healthLabel(p.current_outlook?.health_status))}</p>${p.last_game?`<p class="copy">Last game: ${e(p.last_game.team_score)}–${e(p.last_game.opponent_score)} vs ${e(p.last_game.opponent)} · Final</p>`:''}<p class="copy">Next game: ${e(dateLabel(p.next_game?.date))} · ${e(p.next_game?.opponent)}</p>${p.latest_news?.headline?.toLowerCase().includes(p.player.toLowerCase())?`<p class="copy">${e(dateLabel(p.latest_news.date))} · ${e(p.latest_news.headline)}</p>`:''}</article>`).join('')}</div>`:'<p class="ctdPublishedNote">Weekly outlook could not be loaded.</p>';
 }
 ...
 install();
 document.addEventListener('ctd:player-context-ready',()=>{outlook=window.CTD_PLAYER_OUTLOOK_2026;install();if(document.getElementById('outlookTab')?.classList.contains('active'))renderOutlook()});
...
const read=async u=>{const r=await fetch(u+(u.includes('?')?'&':'?')+'t='+Math.floor(Date.now()/60000),{cache:'no-store',signal:AbortSignal.timeout(20000)});if(!r.ok)throw Error(r.status);return r.json()};
const reportDate=d=>d&&Number.isFinite(Date.parse(d))?new Date(d).toLocaleString():'Date not supplied';
function sharedPlayer(name){
 const roster=findPlayer(Object.values(personnel?.teams||{}).flatMap(t=>t.players||[]),name);
 ...
 for(const story of news){
  ...
  if(trusted&&named&&explicitIR&&Number.isFinite(time)&&time<=Date.now()+300000&&Date.now()-time<7*86400000&&time>(Date.parse(report?.source_updated_at)||0))report={status:'Injured Reserve',source_updated_at:story.published_at,source_url:story.url,source:'Dated news transaction',body_part:null,injury_detail:title};
 }
...
window.CTD_ROLE_WATCH=()=>{
 const rows=[];
 if(!personnel||Number(personnel.week)!==Number(weeklySchedule?.week))return rows;
 for(const t of Object.values(personnel.teams||{}))for(const p of t.players||[]){
  if(!['QB','RB','WR','TE'].includes(p.position))continue;
  const v=sharedPlayer(p.name),report=v.report,status=String(report?.status||'').toLowerCase();
  const longTerm=/^(injured reserve|ir|pup|physically unable to perform|suspended)$/.test(status);
  const age=Date.now()-Date.parse(report?.source_updated_at),fresh=age>=0&&age<=86400000;
  const unavailable=longTerm||(fresh&&/^(out|inactive)$/.test(status));
  ...
  const statLine=(stats,kind)=>!stats?'':[kind==='rushing'&&stats.CAR!=null?stats.CAR+' carries':'',kind==='receiving'&&stats.REC!=null?stats.REC+' catches':'',stats.YDS!=null?stats.YDS+' yards':'',stats.TD!=null?stats.TD+' TD':'',kind==='receiving'&&stats.TGTS!=null?stats.TGTS+' targets':''].filter(Boolean).join(' · ');
  const usage=p.position!=='QB'&&groups.length?[statLine(rush,'rushing'),statLine(rec,'receiving')].filter(Boolean).join(' | '):'';
  const injury=report.body_part||null;
  const explanation=(report.injury_detail?report.injury_detail.replace(/[.!?]+$/,'')+'. ':injury?injury+' injury. ':'Injury details not supplied by the report. ')+(conditional?'If '+p.name+' is ruled out against '+v.opponent+', these teammates could take on more work.':p.name+' is unavailable; these teammates could take on more work against '+v.opponent+'.');
  if(options.length)rows.push({player:p.name,team:t.team,position:p.position,designation:report.status,injury,practice:report.practice_status||null,usage,usageDate:usage?prior.date:null,reported:report.source_updated_at,source:report.source_url||t.source_url,conditional,options,explanation,opponent:v.opponent});
...
async function refreshPlayerContext(){
 ...
 if(values[0].status==='fulfilled')personnel=values[0].value;
 if(values[1].status==='fulfilled')weeklySchedule=values[1].value;
 if(values[2].status==='fulfilled')weeklyProjections=values[2].value;
 if(values[3].status==='fulfilled')profileNews=values[3].value.items||[];
 const players={};for(const t of Object.values(personnel?.teams||{}))for(const p of t.players||[]){const v=sharedPlayer(p.name);players[p.name]={player:p.name,position:p.position,team:p.team,current_outlook:{health_status:v.report? v.report.status+' · Reported '+reportDate(v.report.source_updated_at):'No injury designation supplied; game-day availability unconfirmed.'},next_game:v.game?{opponent:v.opponent,date:v.game.event_start}:null,latest_news:v.news[0]||null};}
 window.CTD_PLAYER_OUTLOOK_2026={players,week:weeklySchedule?.week,generated_at:personnel?.captured_at};
...
const nav=document.getElementById('nav');nav.prepend(nav.querySelector('[data-page="gamesPage"]'));
function profile(name){profileOpenName=name;const p=window.CTD_CANONICAL_PLAYERS_2026?.find(x=>x.name===name);if(!p)return;let d=document.getElementById('ctdPlayerProfile');if(!d){d=document.createElement('dialog');d.id='ctdPlayerProfile';document.body.append(d)}const w=window.CTD_PLAYER_OUTLOOK_2026?.players?.[name];const v=sharedPlayer(name);const health=v.report?v.report.status+' · Reported '+reportDate(v.report.source_updated_at):w?.current_outlook?.health_status;const projected=v.projection?.projections;const news=v.news;const roleRows=window.CTD_ROLE_WATCH().filter(r=>playerKey(r.player)===playerKey(name)||r.options.some(q=>playerKey(q.name)===playerKey(name)));const roleHtml=roleRows.length?'<h3>Role Watch</h3>'+roleRows.map(r=>`<p>${e(r.player)}: ${e(r.designation)}. ${r.conditional?'If he misses this week, watch':'Teammates to watch:'} ${e(r.options.map(q=>q.name).join(', '))}. A larger role is possible; the workload split is not confirmed.</p>`).join(''):'';d.innerHTML=`<button class="btn" data-close-profile>Close profile</button><p>${e(p.pos)} · ${e(v.roster?.team||p.team)}</p><div class="ctdProfileHero">${window.CTD_PLAYER_PORTRAIT?.(p.name,v.roster?.athlete_id,p.pos)||""}<h2>${e(p.name)}</h2></div><h3>Season outlook</h3><p>${e(p.overallWriteup||'No written evaluation has been published.')}</p><dl><dt>Overall rank</dt><dd>${e(p.overall)}</dd><dt>Model value rank</dt><dd>${e(p.trueRank)}</dd><dt>Season projected PPR points</dt><dd>${e(p.projectedPpr)}</dd></dl><p>${e(typeof p.projection==='string'?p.projection:'')}</p><h3>This week</h3><p>${e(health&&health!=='PASS'?health:'No specific health update published; game-day status unconfirmed.')}</p><p>${v.game?e('Week '+v.week+' · '+v.opponent+' · '+reportDate(v.game.event_start)):'Current-week matchup not supplied.'}</p><h3>Weekly stat projections</h3>${projected&&Object.keys(projected).length?`<p>Week ${e(weeklyProjections.week)} · Published ${e(new Date(weeklyProjections.generated_at).toLocaleString())} · Preliminary model estimates</p><dl>${Object.entries(projected).map(([stat,v])=>`<dt>${e(stat.replaceAll("_"," "))}</dt><dd>${Number.isFinite(v.mean)?v.mean.toFixed(1):"Unavailable"}</dd>`).join("")}</dl>`:`<p>${e(v.projection?.reason||'No weekly stat projection is available for this player.')}</p>`}${roleHtml}<h3>Latest player news</h3><p>${news.length?news.map(n=>e(n.headline||n.title)).join('<br>'):v.report?e(v.report.status+' · '+(v.report.body_part||'Injury update')+' · Reported '+reportDate(v.report.source_updated_at)):'No matching player-specific report supplied.'}</p><h3>Last completed game</h3>${w?.last_game?.player_stat_groups?.length?`<p>${e(w.last_game.opponent)} · ${e(new Date(w.last_game.date).toLocaleDateString())}</p>${w.last_game.player_stat_groups.map(g=>`<h4>${e(g.category)}</h4><dl>${Object.entries(g.stats||{}).map(([k,v])=>`<dt>${e(k)}</dt><dd>${e(v)}</dd>`).join('')}</dl>`).join('')}`:'<p>No individual box score has been published for the last completed game.</p>'}`;d.querySelector('[data-close-profile]').onclick=()=>d.close();if(!d.open)d.showModal()}
document.addEventListener('click',ev=>{const p=ev.target.closest('[data-profile]');if(p){ev.preventDefault();profile(p.dataset.profile);return}const b=ev.target.closest('[data-open-game]');if(!b)return;const normalize=s=>String(s||'').replace(/[^a-z0-9]/gi,'').toLowerCase();const i=BET_FEED.games.findIndex(g=>String(g.game_id||g.id)===b.dataset.openGame||normalize(g.away_team+'@'+g.home_team)===normalize(b.dataset.matchup));if(i<0){b.textContent='Game not available in the current week';return}selectedGameIndex=i;activeGameTab='OVERVIEW';showPage('gamesPage',nav.querySelector('[data-page="gamesPage"]'));renderSelectedGame();document.getElementById('gameTitle').scrollIntoView({behavior:'smooth',block:'start'})});
})();
