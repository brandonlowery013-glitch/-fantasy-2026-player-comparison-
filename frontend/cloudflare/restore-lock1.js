...
window.CTD_ROLE_WATCH=()=>{
 ...
 for(const t of Object.values(personnel.teams||{}))for(const p of t.players||[]){
  ...
  const explanation=(injury?injury+' injury. ':'Injury details not supplied by the report. ')+(conditional?'If '+p.name+' is ruled out against '+v.opponent+', these teammates could take on more work.':p.name+' is unavailable; these teammates could take on more work against '+v.opponent+'.');
