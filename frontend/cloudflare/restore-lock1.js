...
function profile(name){profileOpenName=name;const p=window.CTD_CANONICAL_PLAYERS_2026?.find(x=>x.name===name);if(!p)return;let d=document.getElementById('ctdPlayerProfile');if(!d){d=document.createElement('dialog');d.id='ctdPlayerProfile';document.body.append(d)}const w=window.CTD_PLAYER_OUTLOOK_2026?.players?.[name];const v=sharedPlayer(name);const health=v.report?v.report.status+' · Reported '+reportDate(v.report.source_updated_at):w?.current_outlook?.health_status;const projected=v.projection?.projections;const news=v.news;d.innerHTML=`<button class="btn" data-close-profile>Close profile</button><p>${e(p.pos)} · ${e(v.roster?.team||p.team)}</p><div class="ctdProfileHero">${window.CTD_PLAYER_PORTRAIT?.(p.name,v.roster?.athlete_id)||""}<h2>${e(p.name)}</h2></div><h3>Season outlook</h3><p>${e(p.overallWriteup||'No written evaluation has been published.')}</p><dl><dt>Overall rank</dt><dd>${e(p.overall)}</dd><dt>Model value rank</dt><dd>${e(p.trueRank)}</dd><dt>Season projected PPR points</dt><dd>${e(p.projectedPpr)}</dd></dl><p>${e(typeof p.projection==='string'?p.projection:'')}</p><h3>This week</h3><p>${e(health&&health!=='PASS'?health:'No specific health update published; game-day status unconfirmed.')}</p><p>${v.game?e('Week '+v.week+' · '+v.opponent+' · '+reportDate(v.game.event_start)):'Current-week matchup not supplied.'}</p><h3>Weekly stat projections</h3>${projected&&Object.keys(projected).length?`<p>Week ${e(weeklyProjections.week)} · Published ${e(new Date(weeklyProjections.generated_at).toLocaleString())} · Preliminary model estimates</p><dl>${Object.entries(projected).map(([stat,v])=>`<dt>${e(stat.replaceAll("_"," "))}</dt><dd>${Number.isFinite(v.mean)?v.mean.toFixed(1):"Unavailable"}</dd>`).join("")}</dl>`:`<p>${e(v.projection?.reason||'No weekly stat projection is available for this player.')}</p>`}<h3>Latest player news</h3><p>${news.length?news.map(n=>e(n.headline||n.title)).join('<br>'):v.report?e(v.report.status+' · '+(v.report.body_part||'Injury update')+' · Reported '+reportDate(v.report.source_updated_at)):'No matching player-specific report supplied.'}</p><h3>Last completed game</h3>${w?.last_game?.player_stat_groups?.length?`<p>${e(w.last_game.opponent)} · ${e(new Date(w.last_game.date).toLocaleDateString())}</p>${w.last_game.player_stat_groups.map(g=>`<h4>${e(g.category)}</h4><dl>${Object.entries(g.stats||{}).map(([k,v])=>`<dt>${e(k)}</dt><dd>${e(v)}</dd>`).join('')}</dl>`).join('')}`:'<p>No individual box score has been published for the last completed game.</p>'}`;d.querySelector('[data-close-profile]').onclick=()=>d.close();if(!d.open)d.showModal()}
