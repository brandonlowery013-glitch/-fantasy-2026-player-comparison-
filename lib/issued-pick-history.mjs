...
export function captureHistory({ledger, schedule, gameRecs, propRecs, propSnapshots, now, sourceCommit=null}) {
 ...
 function append(row){
  ...
  const record={...row,season,week,kickoff,captured_at:now,source_commit:sourceCommit,record_kind:'PUBLISHED_FEED_CANDIDATE',mode:'SHADOW_ONLY',actionable:false,fingerprint,revision:(old?.revision||0)+1,previous_record_id:old?.record_id||null};
 ...
 for(const old of [...latest.values()])if(old.week===week&&!seen.has(old.stream_id)&&old.decision==='PICK'){
  const {record_id,fingerprint,revision,previous_record_id,captured_at,source_commit,record_kind,mode,actionable,kickoff,...row}=old;
