...
const shadowProjection=JSON.parse(fs.readFileSync('guardrails/step3b-shadow-projection-context-162.json','utf8'));
const shadowDownstream=JSON.parse(fs.readFileSync('guardrails/step3b-shadow-projection-downstream-162.json','utf8'));
const shadowReview=JSON.parse(fs.readFileSync('guardrails/step3b-shadow-overall-review-queue-162.json','utf8'));
...
add(players.length===162?'PASS':'BLOCKER','ACTIVE_UNIVERSE_162',`player shards contain ${players.length} active players`);
add(unique.size===162?'PASS':'BLOCKER','UNIQUE_IDENTITIES_162',`${unique.size} unique player names`);

if(legacyProjection.players_checked!==162)add('INFO','LEGACY_PROJECTION_AUDIT_153',`legacy saved projection audit checked ${legacyProjection.players_checked}; retained as historical artifact, not current evidence`);
if(legacyDownstream.players_checked!==162)add('INFO','LEGACY_DOWNSTREAM_AUDIT_153',`legacy saved downstream audit checked ${legacyDownstream.players_checked}; retained as historical artifact, not current evidence`);

add(shadowProjection.players_checked===162&&shadowProjection.shadow_only===true&&shadowProjection.live_player_files_modified===false?'PASS':'BLOCKER','SHADOW_PROJECTION_162','fresh shadow projection audit must cover all 162 and make no live mutations');
add(shadowDownstream.players_checked===162&&shadowDownstream.shadow_only===true&&shadowDownstream.live_player_files_modified===false?'PASS':'BLOCKER','SHADOW_DOWNSTREAM_162','fresh shadow downstream audit must cover all 162 and make no live mutations');
add(shadowReview.players_checked===162&&shadowReview.shadow_only===true&&shadowReview.published===false?'PASS':'BLOCKER','SHADOW_REVIEW_QUEUE_162','fresh 162 Overall review queue exists and remains unpublished');
...
if(decisions.universe_size!==162||decisions.approval_status!=='APPROVED_FOR_APPLY')add('PASS','OLD_37_DECISIONS_BLOCKED','Aug 23 decision set lacks fresh 162 approval metadata and is therefore blocked from application');
...
const report={
  ...
  legacy_153_artifacts_retained_as_history:{projection_context_players_checked:legacyProjection.players_checked,projection_downstream_players_checked:legacyDownstream.players_checked,overall_decisions_as_of:decisions.as_of??null,overall_decisions_count:decisions.decisions?.length??0},
  ...
  blockers,findings,
  next_required_actions:blockers.length?['Resolve reconciliation blockers before model promotion.']:['Use the reconciled 162-player shadow baseline to continue Step 3B Bayesian validation.','Do not publish any proposed rank or projection movement before the user review gate.']
};
