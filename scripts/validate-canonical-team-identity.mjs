...
async function getJson(url){const r=await fetch(url,{headers:{'user-agent':'fantasy-2026-team-identity-validator/1.1','accept':'application/json'}});if(!r.ok)throw new Error(`${url} -> ${r.status}`);return r.json();}
...
const report={generated_at:new Date().toISOString(),result:counts.MISMATCH||counts.UNKNOWN_CANONICAL_TEAM||counts.AMBIGUOUS_LIVE_TEAM?'FAIL':'PASS',universe:expected,verified:rows.filter(x=>x.live_teams.length===1).length,counts,source_quality:{depth_teams_checked:32,depth_failures:depthFailures,roster_failures:rosterFailures,roster_optional:true},mismatches:rows.filter(x=>x.status==='MISMATCH'),unknown_canonical:rows.filter(x=>x.status==='UNKNOWN_CANONICAL_TEAM'),ambiguous:rows.filter(x=>x.status==='AMBIGUOUS_LIVE_TEAM'),unresolved:rows.filter(x=>x.status==='UNRESOLVED_LIVE_TEAM'),policy:'RUNTIME_PLAYER_SHARD_TEAM_IDENTITY_MUST_MATCH UNIQUE CURRENT ESPN DEPTH/ROSTER TEAM WHEN RESOLVED; DEPTH SOURCE IS REQUIRED, ROSTER IS OPTIONAL SUPPLEMENT'};
