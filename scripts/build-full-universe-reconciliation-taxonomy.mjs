...
function evidenceFact(m){return String(m?.matched_context||m?.description||m?.headline||'').trim()||null;}
...
const provenanceControl={headline:'NFL news roundup: unrelated lead item',matched_context:'Ashton Jeanty is still on the mend from an ankle injury and will be assessed for Week 1'};
if(evidenceFact(provenanceControl)!==provenanceControl.matched_context)throw new Error('PROVENANCE_REGRESSION: matched player context did not outrank roundup headline');
...
for(const p of players){
  ...
  const teamContextOnly=material&&sigs.length>0&&sigs.every(x=>x.evidence_lane==='material_team_context_signals');
  const evidenceText=sigs.map(x=>`${x.matched_context||''} ${x.description||''} ${x.headline||''}`).filter(Boolean).join(' ');
  ...
  else if(material&&teamContextOnly)taxonomy='MATERIAL_HOLD';
...
const badProvenance=rows.filter(x=>x.meaningful_evidence&&x.evidence_summary?.[0]?.matched_context&&x.changed_fact!==x.evidence_summary[0].matched_context);if(badProvenance.length)throw new Error(`MATCHED_CONTEXT_PROVENANCE_LOST: ${badProvenance.map(x=>x.player).join(', ')}`);
const badTeamDirectional=rows.filter(x=>x.team_context_only&&(x.taxonomy==='POSITIVE_HOLD'||x.taxonomy==='NEGATIVE_HOLD'));if(badTeamDirectional.length)throw new Error(`TEAM_CONTEXT_DIRECTIONAL_HOLD: ${badTeamDirectional.map(x=>x.player).join(', ')}`);
const counts=Object.fromEntries([...allowed].map(k=>[k,rows.filter(x=>x.taxonomy===k).length]));
for(const x of ledger.players||[]){const row=rows.find(r=>r.player===x.player);if(row){x.reconciliation_taxonomy=row.taxonomy;x.reconciliation={meaningful_evidence:row.meaningful_evidence,team_context_only:row.team_context_only,changed_fact:row.changed_fact,evidence_summary:row.evidence_summary,implicated_components:row.implicated_components,score_delta:row.score_delta,proposed_true_value_rank:row.proposed_true_value_rank,connected_player_effects:row.connected_player_effects,market_impact:row.market_impact};}}
ledger.reconciliation_taxonomy_schema={version:'1.5.0',allowed:[...allowed],generated_at:new Date().toISOString(),coverage:rows.length,validation:'MATERIAL_HOLD_REQUIRES_DIRECT_OR_CONNECTED_FANTASY_RELEVANT_EVIDENCE; LOCAL MATCHED CONTEXT PRESERVED; TEAM-CONTEXT-ONLY EVIDENCE CANNOT CREATE DIRECTIONAL PLAYER HOLD'};
write('guardrails/current-football-review.json',ledger);
const report={schema_version:'1.5.0',generated_at:new Date().toISOString(),authoritative:false,mutation_policy:'PROPOSAL_ONLY_NO_CANONICAL_WRITES',universe:{players:expected,shards},counts,validation:{exact_universe:true,unique_players:true,silent_material_evidence:0,directional_numeric_consistency:true,cross_team_context_guard:true,non_material_operational_guard:true,material_hold_requires_validated_evidence:true,incidental_mention_guard:true,matched_context_provenance_guard:true,team_context_directionality_guard:true,phantom_material_holds:0},rows};
