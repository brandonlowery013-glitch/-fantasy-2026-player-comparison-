...
for(const x of ledger.players||[]){const row=rows.find(r=>r.player===x.player);if(row){x.reconciliation_taxonomy=row.taxonomy;x.reconciliation={meaningful_evidence:row.meaningful_evidence,changed_fact:row.changed_fact,evidence_summary:row.evidence_summary,implicated_components:row.implicated_components,score_delta:row.score_delta,proposed_true_value_rank:row.proposed_true_value_rank,connected_player_effects:row.connected_player_effects,market_impact:row.market_impact};}}
ledger.reconciliation_taxonomy_schema={version:'1.4.0',allowed:[...allowed],generated_at:new Date().toISOString(),coverage:rows.length,validation:'MATERIAL_HOLD_REQUIRES_DIRECT_OR_CONNECTED_FANTASY_RELEVANT_EVIDENCE_AND_PRESERVES_LOCAL_MATCHED_CONTEXT'};
write('guardrails/current-football-review.json',ledger);
const report={schema_version:'1.4.0',generated_at:new Date().toISOString(),authoritative:false,mutation_policy:'PROPOSAL_ONLY_NO_CANONICAL_WRITES',universe:{players:expected,shards},counts,validation:{exact_universe:true,unique_players:true,silent_material_evidence:0,directional_numeric_consistency:true,cross_team_context_guard:true,non_material_operational_guard:true,material_hold_requires_validated_evidence:true,incidental_mention_guard:true,matched_context_provenance_guard:true,phantom_material_holds:0},rows};
