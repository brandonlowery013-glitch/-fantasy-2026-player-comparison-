...
// Regression guards for exact contamination classes seen in the 2026-09-04 production reconciliation.
if(relevantSignal({n:'Ashton Jeanty',t:'Las Vegas Raiders'},{headline:"Titans WR Wan'Dale Robinson not believed to have suffered concussion",team:'TEN'},'material_team_context_signals'))throw new Error('CROSS_TEAM_REGRESSION: Jeanty accepted Titans context');
if(relevantSignal({n:'Jeremiyah Love',t:'Atlanta Falcons'},{headline:'Cardinals agree to terms with WR Michael Wilson on three-year extension',team:'ARI'},'material_team_context_signals'))throw new Error('CROSS_TEAM_REGRESSION: Love accepted Cardinals context');
if(relevantSignal({n:'DK Metcalf',t:'Pittsburgh Steelers'},{headline:'Dolphins trading WR Tutu Atwell back to Rams; Packers signing TE Jonnu Smith',team:'MIA'},'material_team_context_signals'))throw new Error('CROSS_TEAM_REGRESSION: Metcalf accepted unrelated team context');
if(relevantSignal({n:'George Kittle',t:'San Francisco 49ers'},{headline:'Niners arrive in Melbourne after 15-hour flight',team:'SF'},'material_team_context_signals'))throw new Error('NON_MATERIAL_REGRESSION: travel story accepted as fantasy evidence');
if(relevantSignal({n:'Travis Kelce',t:'Kansas City Chiefs'},{headline:"Melbourne's MCG is ready for first-ever NFL game with Rams-49ers"},'material_team_context_signals'))throw new Error('NON_MATERIAL_REGRESSION: Kelce accepted unrelated stadium story');
if(hasValidatedMaterialBasis({status:'BLOCKED_MISSING_QUANTITATIVE_EVIDENCE'},[]))throw new Error('STATUS_INHERITANCE_REGRESSION: blocked recalculation without validated evidence remained material');
if(hasValidatedMaterialBasis(null,[]))throw new Error('STATUS_INHERITANCE_REGRESSION: empty evidence became material');
...
if(!hasValidatedMaterialBasis({status:'BLOCKED_MISSING_QUANTITATIVE_EVIDENCE'},[positiveControl]))throw new Error('POSITIVE_CONTROL_REGRESSION: validated evidence did not retain material review');
...
for(const p of players){
  ...
  if(q?.status==='NUMERIC_TV_PROPOSAL'){
    taxonomy=Number(q.score_delta)>0?'UP':Number(q.score_delta)<0?'DOWN':'MATERIAL_HOLD';
  }else if(material&&q?.status==='BLOCKED_MISSING_QUANTITATIVE_EVIDENCE'){
    taxonomy='MATERIAL_HOLD';
  }else if(material){
    taxonomy=sent==='POSITIVE'?'POSITIVE_HOLD':sent==='NEGATIVE'?'NEGATIVE_HOLD':'MATERIAL_HOLD';
  }
