...
if(c.season!==2026)blocked.push('season must be 2026');if(c.mode!=='SHADOW_ONLY'||c.actionable!==false)blocked.push('must remain SHADOW_ONLY/non-actionable');if(c.player_universe!==read('MODEL_SOURCE_OF_TRUTH.json').active_player_model)blocked.push('player universe must match canonical model');if(c.fair_market_method!=='PROPORTIONAL_TWO_WAY')blocked.push('fair market method drift');
...
const text=JSON.stringify(c.locked_rules||[]).toLowerCase();for(const phrase of ['model_source_of_truth.json','de-vigged','push','close','mutate'])if(!text.includes(phrase))blocked.push(`locked rules missing ${phrase}`);
