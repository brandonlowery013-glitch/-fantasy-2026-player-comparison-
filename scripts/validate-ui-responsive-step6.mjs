...
check('main mobile grids collapse',has(main,'.grid,.split,.profileGrid{grid-template-columns:1fr}'));
...
try{changed=execSync(`git diff --name-only ${BASE}...HEAD`,{encoding:'utf8'}).trim().split(/\r?\n/).filter(Boolean);}catch(e){
  check('Step 6 diff available',false,String(e.message));
}
...
console.log(JSON.stringify({status:closure.status,checks:checks.length,failures:failures.map(x=>x.name),changed_paths:changed},null,2));
