...
function synthetic(){return {rows:[{player:"Ja'Marr Chase",team:'CIN',position:'WR',designation:'Q',status:'Questionable',practice_status:'Limited',body_part:'Knee',source:'self-test'},{player:'Rashee Rice',team:'KC',position:'WR',designation:'SSPD',status:'Suspended',practice_status:null,body_part:null,source:'self-test'}],errors:[]};}
...
const report={result:result.errors.length?'PARTIAL':'PASS',changed:isChanged,players:next.players.length,sspd:next.players.filter(x=>x.designation==='SSPD').length,errors:result.errors};
...
if(self&&(!next.players.some(x=>x.player==="Ja'Marr Chase"&&x.designation==='Q')||!next.players.some(x=>x.designation==='SSPD')||!isChanged))process.exit(1);
