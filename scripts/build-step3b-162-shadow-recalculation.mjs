...
for(const p of players){
  ...
  const proposedS=Math.round((proposedPD*.35+Number(p.ce)*.20+Number(p.r)*.15+Number(p.e)*.10+Number(p.a)*.10+Number(p.rl)*.05+Number(p.su)*.05)*1000)/1000;
  ...
  rows.push({
    ...
    live_expected_production:p.pd,shadow_expected_production:proposedPD,live_true_value_score:p.s,
    shadow_true_value_score:proposedS,old_production_tier:oldTier,new_production_tier:newTier,
    material_legacy_history_change:Math.abs(delta)>=8||oldTier!==newTier
  });
}

const shadowRank=[...rows].sort((a,b)=>b.shadow_true_value_score-a.shadow_true_value_score||a.live_overall_rank-b.live_overall_rank||a.name.localeCompare(b.name));
shadowRank.forEach((r,i)=>r.shadow_true_value_rank=i+1);
for(const r of rows){
  r.shadow_true_value_rank_move=(r.live_true_value_rank??r.shadow_true_value_rank)-r.shadow_true_value_rank;
  r.shadow_overall_review=r.material_legacy_history_change&&(Math.abs(r.shadow_true_value_rank_move)>=3||Math.abs(r.shadow_projection_delta)>=12||r.old_production_tier!==r.new_production_tier);
}
...
fs.writeFileSync('guardrails/step3b-shadow-projection-context-162.json',JSON.stringify({
  ...
  method:'Read-only reproduction of the existing historical-context projection recalibration across the current 162-player universe. This is a reconciliation baseline, not the final Bayesian model.',
  changes:rows
},null,2)+'\n');
...
fs.writeFileSync('guardrails/step3b-shadow-overall-review-queue-162.json',JSON.stringify({
  ...
  note:'Proposal queue only. Overall ranks remain unchanged until the user review gate and explicit approval.',
  review_queue:review
},null,2)+'\n');

console.log(`162 shadow recalculation PASS — direct history ${162-noHistory.length}, no direct history ${noHistory.length}, material ${material.length}, Overall review ${review.length}`);
