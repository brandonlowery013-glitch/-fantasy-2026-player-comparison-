...
function catsFor(r){const tr=transitionMap.get(r.player),basis=tr?.chronological_development?.current_season_state?.current_state_basis||r.current_season_basis||null;return {basis,categories:[...(basis?.categories||[])],direction:basis?.direction||null};}
...
const rows=audit.rows.map(r=>{
  const {basis,categories,direction}=catsFor(r),prior=Boolean(r.previous_flag),statusRefresh=canonicalStatusRefresh(r),directional=validDirectional.has(direction)&&categories.length>0,components=implicated(categories);
