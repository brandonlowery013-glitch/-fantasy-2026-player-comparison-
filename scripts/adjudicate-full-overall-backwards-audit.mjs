...
const bindingMap=new Map((bindingInput.decisions||[]).map(x=>[x.player,x]));
...
if(bindingMap.size!==31)throw new Error(`subject-binding decision count ${bindingMap.size}/31`);
const unused=[...bindingMap.keys()].filter(x=>!usedBinding.has(x));
if(unused.length)throw new Error(`subject-binding decisions did not match directional audit rows: ${unused.join(', ')}`);
...
const report={schema_version:'1.2.0',generated_at:new Date().toISOString(),season:source.season,universe:166,authority:'EXPLICIT_SUBJECT_BINDING_ADJUDICATION_NO_AUTOMATIC_NUMERIC_OR_RANK_MUTATION',coverage:{reviewed:rows.length,expected:166,complete:rows.length===166,order:'OVERALL_166_TO_1',subject_binding_decisions:bindingMap.size,subject_binding_unresolved:unresolved.length},policy:{prior_flags:'CLOSE AFTER EXPLICIT FULL-UNIVERSE READJUDICATION WHEN NO DIRECTIONAL CONTRADICTION REMAINS',status:'CLOSED CAMP/PRESEASON WORDING IS STATUS_REFRESH_ONLY UNLESS UNACCOUNTED DIRECTIONAL EVIDENCE EXISTS',components:'ONLY VERIFIED PLAYER-OWNED FOOTBALL EVIDENCE MAY REOPEN TRUE-VALUE COMPONENTS; NUMERIC DELTAS REQUIRE THE QUANTITATIVE PIPELINE',overall:'OVERALL RANK DOES NOT REOPEN UNTIL COMPONENT RECALCULATION OR A SEPARATE ACTIONABLE-RANK FOOTBALL RULE PRODUCES A VALID CHANGE',directional_fail_closed:'AN UP/DOWN SIGNAL MUST BE EXPLICITLY REBOUND, REJECTED, MARKED CONTEXT-ONLY, OR ACCOUNTED; IT MAY NOT SILENTLY FALL THROUGH TO HOLD.'},counts,component_reopens:componentReopens,rejected_bindings:rejectedBindings,context_no_numeric:contextNoNumeric,accounted_prior:accountedPrior,status_refreshes:statusRefreshes,rows};
write('analysis/full-overall-backwards-adjudication-current.json',report);
const md=['# Full Overall Backwards Audit — Subject-Bound Adjudication','',`Coverage: ${rows.length}/166, Overall #166 → #1.`,`Explicit subject-binding decisions: ${bindingMap.size}.`,`Unresolved subject-binding signals: ${unresolved.length}.`,`Component reopens pending quantitative evidence: ${componentReopens.length}.`,'', '| Overall | Player | Pos | Final disposition | Direction | Components |','|---:|---|---|---|---|---|'];for(const r of rows)md.push(`| ${r.overall_rank} | ${r.player.replace(/\|/g,'/')} | ${r.position} | ${r.final_disposition} | ${r.binding_adjudication?.direction||r.review_direction||r.subject_bound_current_basis?.direction||'NONE'} | ${r.implicated_components.join(', ')} |`);fs.writeFileSync(path.join(root,'analysis/full-overall-backwards-adjudication-current.md'),md.join('\n')+'\n');
console.log(JSON.stringify({result:'PASS',coverage:'166/166',counts,subject_binding_decisions:bindingMap.size,subject_binding_unresolved:unresolved.length,component_reopens:componentReopens.map(x=>({player:x.player,direction:x.direction,components:x.implicated_components})),rejected_bindings:rejectedBindings.map(x=>x.player),context_no_numeric:contextNoNumeric.map(x=>x.player),accounted_prior:accountedPrior.map(x=>x.player),status_refreshes:statusRefreshes.map(x=>x.player)},null,2));
