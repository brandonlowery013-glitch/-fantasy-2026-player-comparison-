...
function build({schedule,gameRecs,propRecs,parlays}){
  ...
  if(week==null)return {schema_version:'1.0.0',season:2026,week:null,status:'WAITING_FOR_CURRENT_WEEK',generated_at:new Date().toISOString(),games:[],props:[],parlays:[],blocked:[]};
  if(blocked.length)return {schema_version:'1.0.0',season:2026,week:Number(week),status:'WAITING_FOR_CURRENT_WEEK_OUTPUTS',generated_at:new Date().toISOString(),games:[],props:[],parlays:[],blocked};
  ...
  const legEvent=new Map((parlays?.eligible_legs||[]).filter(x=>x.source_type==='PLAYER_PROP').map(x=>[`${x.player}|${x.stat}|${x.line}|${x.sportsbook}`,x.event_id]));
  ...
  return {schema_version:'1.0.0',season:2026,week:Number(week),status:'READY',generated_at:new Date().toISOString(),games,props:propRows.slice(0,100),parlays:parlayRows,blocked:[],provenance:{schedule_status:schedule.status||null,game_recommendation_status:gameRecs.status||null,prop_recommendation_status:propRecs.status||null,parlay_status:parlays.status||null}};
}

function synthetic(){return {schedule:{season:2026,week:3,status:'VERIFIED',games:{g1:{away_team:'A',home_team:'B',status:'Scheduled'}}},gameRecs:{week:3,status:'SHADOW_ONLY',games:{g1:{week:3,away_team:'A',home_team:'B',football_projection:{home_win_probability:.57},current_recommendations:{snapshot_id:'s',spread:{decision:'PICK',selection:'B -2.5',confidence:'MODERATE',expected_value:.08,probability_edge:.05},total:{decision:'PASS'},moneyline:{decision:'PASS'}},snapshot_evaluations:[{snapshot_id:'s',market:{home_spread:-2.5,total:45.5,home_moneyline:-130,away_moneyline:110}}]}}},propRecs:{week:3,players:{}},parlays:{week:3,status:'READY',eligible_leg_count:0,eligible_legs:[],model_picks:[]}};}
...
const out=build(src);
if(self){
  if(out.status!=='READY'||out.week!==3||out.games.length!==1)throw new Error('self-test current-week feed failed');
  const stale=build({...src,gameRecs:{...src.gameRecs,week:2}});
  if(stale.status!=='WAITING_FOR_CURRENT_WEEK_OUTPUTS'||stale.games.length)throw new Error('self-test stale-week isolation failed');
}
if(!self)write('data/market/final-betting-ui-feed-2026.json',out);
console.log(JSON.stringify({result:out.blocked?.length?'BLOCKED':'PASS',week:out.week,status:out.status,games:out.games.length,props:out.props.length,parlays:out.parlays.length,blocked:out.blocked},null,2));
