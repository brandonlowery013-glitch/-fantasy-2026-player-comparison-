...
let before=0,after=0,teamContextAfter=0;
for(const p of ledger.players||[]){
  before+=(p.material_news_signals||[]).length;
  const direct=dedupe((p.news_mentions||[]).map(m=>qualify(p.player,m)).filter(Boolean));
  const teamContext=dedupe((p.team_context_mentions||[]).map(qualifyTeamContext).filter(Boolean));
  p.direct_material_news_signals=direct;
  p.material_team_context_signals=teamContext;
  p.material_news_signals=dedupe([...direct,...teamContext]);
  after+=direct.length;teamContextAfter+=teamContext.length;
}
