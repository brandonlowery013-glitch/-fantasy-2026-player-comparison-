...
if(comebackEvent){
  ...
  const scoreStates=objs.filter(o=>{const period=Number(o.period?.number??o.period??o.quarter),hs=Number(o.homeScore??o.home_score),as=Number(o.awayScore??o.away_score);return Number.isFinite(period)&&period>=4&&Number.isFinite(hs)&&Number.isFinite(as);});
...
const hasExplicitTimezone=[...venueKeys].some(k=>k.includes('timezone'));
...
const sourceDecisions=[
  {indicator:'trailed_14_plus_fourth_quarter_then_won',status:comebackProbe.chronological_score_state_found?'SOURCE_RESOLVED':'SOURCE_NOT_CONFIRMED',source:'ESPN public NFL game summary play-by-play',evidence:comebackProbe,projection_weight:0,next_action:comebackProbe.chronological_score_state_found?'Historical backfill may be built from the verified score-state chronology before any model promotion.':'Keep source-blocked; no model authority.'},
  {indicator:'international_game_recovery',status:internationalEvents.length>0&&countryFieldCoverage>0.9?'SOURCE_RESOLVED':'SOURCE_NOT_CONFIRMED',source:'ESPN public NFL scoreboard competition.venue.address.country',evidence:{events_scanned:events.length,country_field_coverage:countryFieldCoverage,international_events_found:internationalEvents.length,examples:internationalEvents.slice(0,5).map(e=>({season:e.season,week:e.week,name:e.name,country:e.venue?.address?.country}))},projection_weight:0,next_action:'Country flag is source-resolved; historical recovery classification may be rebuilt and revalidated before promotion.'},
  {indicator:'west_to_east_early_kick',status:hasExplicitTimezone&&hasCoordinates?'SOURCE_RESOLVED':'ZERO_AUTHORITY_SOURCE_INSUFFICIENT',source:'Current locked ESPN scoreboard venue metadata',evidence:{events_scanned:events.length,explicit_timezone_field_found:hasExplicitTimezone,coordinate_field_found:hasCoordinates,venue_keys:[...venueKeys].sort()},projection_weight:0,next_action:'Requires a provenance-backed stadium/team timezone or longitude dataset plus kickoff-local-time mapping; do not infer from team names.'},
  {indicator:'high_snap_load_previous_game',status:snapProbe.snap_field_found?'SOURCE_RESOLVED':'ZERO_AUTHORITY_SOURCE_INSUFFICIENT',source:'Current locked ESPN game summary plus existing skill-position history',evidence:snapProbe,projection_weight:0,next_action:'Requires complete comparable snap/workload history; existing partial skill-position usage cannot be treated as complete game workload.'}
];
const unresolved=sourceDecisions.filter(x=>x.status==='SOURCE_NOT_CONFIRMED');
const out={schema_version:'STEP4_SITUATIONAL_SOURCE_RESOLUTION_1.0.0',history_window:years,events_scanned:events.length,missing_is_unknown:true,sportsbook_or_adp_used:false,source_decisions:sourceDecisions,source_resolved:sourceDecisions.filter(x=>x.status==='SOURCE_RESOLVED').map(x=>x.indicator),zero_authority_insufficient_source:sourceDecisions.filter(x=>x.status==='ZERO_AUTHORITY_SOURCE_INSUFFICIENT').map(x=>x.indicator),status:unresolved.length?'REVIEW_REQUIRED':'ALL_FOUR_EXPLICITLY_RESOLVED_OR_ZERO_AUTHORITY'};
