...
const edgeBy = new Map(edgeRows.map(x=>[x.player,x]));
const marketBy = new Map(marketRows.map(x=>[x.player,x]));
...
for (const [rankField, prefixField] of [['pr','o'],['tp','tr']]) {
  ...
  for (const name of coreNames) {
    ...
    rows.push({name,num:+num,rank:c[prefixField]});
    byPos.set(pos,rows);
  }
...
assert(!forbiddenImports.some(r=>r.test(marketBuilder)), 'Step 3 market-value builder must not import sportsbook/Vegas/prop authority');
