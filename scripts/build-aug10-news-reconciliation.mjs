...
for(const p of ledger.players||[]){
  ...
  for(const m of mentions){const key=`${m._teamContext?'TEAM':'DIRECT'}|${m.url||`${m.source}|${m.headline}|${m.description}`}`;if(seen.has(key))continue;seen.add(key);const isTeamContext=Boolean(m._teamContext);events.push({date:new Date(m._t).toISOString(),source:m.source,headline:m.headline||m.description||'',url:m.url||null,team:m.team||p.current_team||p.team||null,context_scope:isTeamContext?'TEAM_OFFENSE':'DIRECT_PLAYER',context_lanes:m.context_lanes||[],context_lane_scope:m.context_lane_scope||null,promote_for_player_review:Boolean(m.promote_for_player_review),direct_modeled_players:m.direct_modeled_players||[],category:category(m._text),direction:direction(m._text,isTeamContext),already_reflected:isTeamContext?false:reflected(p.player,m._text)});}
