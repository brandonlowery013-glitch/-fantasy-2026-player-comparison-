...
const role=/\b(role|workload|target|targets|route|routes|snap|snaps|carry|carries|touch|touches|reps|first team|starter|starting|red zone|goal line|committee|timeshare|depth chart|wr1|rb1|te1|qb1|chemistry|timing|connection|trust|offense|scheme)\b/i;
...
function direction(t){if(recovery.test(t)&&!negative.test(t))return'UP';if(negative.test(t)&&!recovery.test(t))return'DOWN';if(positiveRole.test(t)&&!negative.test(t))return'UP';if(transaction.test(t))return'CONTEXT';return'REVIEW';}
...
for(const p of ledger.players||[]){
  const mentions=(p.material_news_signals||[]).map(m=>({...m,_t:dateOf(m),_text:textOf(m)})).filter(m=>m._t!==null&&m._t>=START&&m._t<=END).sort((a,b)=>a._t-b._t);
  if(!mentions.length)continue;
  const seen=new Set();const events=[];
  for(const m of mentions){const key=m.url||`${m.source}|${m.headline}|${m.description}`;if(seen.has(key))continue;seen.add(key);events.push({date:new Date(m._t).toISOString(),source:m.source,headline:m.headline||m.description||'',url:m.url||null,category:category(m._text),direction:direction(m._text),already_reflected:reflected(p.player,m._text)});}
  const cats={};for(const e of events)(cats[e.category]??=[]).push(e);
  ...
  rows.push({player:p.player,event_count:events.length,current_category_states:current,missing_current_states:missing.length,suggestion,events});
}
rows.sort((a,b)=>b.missing_current_states-a.missing_current_states||b.event_count-a.event_count||a.player.localeCompare(b.player));
const summary={generated_at:new Date().toISOString(),window_start:new Date(START).toISOString(),window_end:new Date(END).toISOString(),players_reviewed:(ledger.players||[]).length,players_with_window_events:rows.length,players_with_missing_current_states:rows.filter(r=>r.missing_current_states>0).length,suggestion_counts:Object.fromEntries([...new Set(rows.map(r=>r.suggestion))].map(k=>[k,rows.filter(r=>r.suggestion===k).length])),canonical_mutations:0,governance:'PROPOSAL_ONLY',players:rows};
write('analysis/aug10-news-reconciliation.json',summary);
const md=['# Aug. 10-current historical news reconciliation','',`Generated: ${summary.generated_at}`,`Window: ${summary.window_start} -> ${summary.window_end}`,`Universe: ${summary.players_reviewed}`,`Players with qualifying events: ${summary.players_with_window_events}`,`Players with current states not obviously reflected in canonical notes: ${summary.players_with_missing_current_states}`,'','| Player | Events | Missing current states | Suggestion |','|---|---:|---:|---|',...rows.map(r=>`| ${r.player.replaceAll('|','\\|')} | ${r.event_count} | ${r.missing_current_states} | ${r.suggestion} |`),'','No canonical files were changed. Historical events are reconstructed oldest-to-newest; only the latest state per category is treated as current while older events remain context.'];fs.writeFileSync(path.join(root,'analysis/aug10-news-reconciliation.md'),md.join('\n')+'\n');
console.log(JSON.stringify({result:'PASS',window_start:summary.window_start,players_with_window_events:summary.players_with_window_events,players_with_missing_current_states:summary.players_with_missing_current_states,suggestion_counts:summary.suggestion_counts,top:rows.slice(0,30).map(r=>({player:r.player,missing:r.missing_current_states,suggestion:r.suggestion}))},null,2));