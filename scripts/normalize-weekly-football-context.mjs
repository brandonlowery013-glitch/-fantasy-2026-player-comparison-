...
const activeCount=Number(sourceOfTruth.active_player_model),activeShards=Number(sourceOfTruth.runtime_player_shards);
...
for(let i=0;i<activeShards;i++)for(const p of read(`players${i}.json`)){
  const key=norm(p.n);
  if(liveByNorm.has(key))throw new Error(`Duplicate normalized canonical player: ${p.n}`);
  liveByNorm.set(key,{name:p.n,position:String(p.p||'').toUpperCase()});
}
if(liveByNorm.size!==activeCount)throw new Error(`Active universe contract mismatch: expected ${activeCount}, found ${liveByNorm.size}`);

function synthetic(){
  const now=Date.now(); const iso=h=>new Date(now-h*3600000).toISOString();
  return {schema_version:'self-test',season:2026,week:1,status:'SELF_TEST',captured_at:new Date(now).toISOString(),sportsbook_inputs_used:false,players:{
    ...
    SELF_TEST_TE:{position:'TE',expected_active:null,availability_status:'UNKNOWN',signals:{role:{source:'self-test utilization',captured_at:iso(2),cohort:'TE_RECEIVING',stat_adjustments:{targets:{mean_pct:.06},receptions:{mean_pct:.05}}},injury:{source:'self-test official injury',captured_at:iso(13),stat_adjustments:{targets:{mean_pct:-.04}}}}}
  }};
}

const selfTest=process.argv.includes('--self-test'); const src=selfTest?synthetic():read(rawPath); const now=Date.now();
const blocked=[]; const review=[];
...
if(!selfTest){
  const inputByNorm=new Map();
  for(const inputName of Object.keys(src.players||{})){
    const key=norm(inputName);
    if(inputByNorm.has(key))blocked.push(`duplicate normalized weekly-context player: ${inputName}`);
    inputByNorm.set(key,inputName);
  }
  const missing=[...liveByNorm.entries()].filter(([key])=>!inputByNorm.has(key)).map(([,p])=>p.name);
  const extra=[...inputByNorm.entries()].filter(([key])=>!liveByNorm.has(key)).map(([,name])=>name);
  if(inputByNorm.size!==activeCount||missing.length||extra.length)blocked.push(`weekly context roster mismatch: expected ${activeCount}, found ${inputByNorm.size}; missing=[${missing.join(', ')}]; extra=[${extra.join(', ')}]`);
}

const players={};let currentSignals=0,staleSignals=0,invalidSignals=0,currentRoleCohorts=0,staleRoleCohorts=0,unknownAvailability=0;
for(const [inputName,p] of Object.entries(src.players||{})){
  ...
  if(!selfTest){
    ...
    if(!live){blocked.push(`${inputName} is not in authoritative ${activeCount}-player universe`);continue;}
    name=live.name;if(live.position&&live.position!==suppliedPos){blocked.push(`${inputName} position mismatch: ${suppliedPos} vs live ${live.position}`);continue;}
  }
  const outSignals={};let playerReview=!availabilityKnown;
  if(!availabilityKnown){unknownAvailability++;review.push(`${name} availability UNKNOWN — no active/inactive state inferred`);}
  for(const [signalName,s] of Object.entries(p.signals||{})){
    ...
    for(const [stat,a] of Object.entries(s.stat_adjustments||{})){
      ...
      const x={}; if(a.mean_pct!=null){if(!finite(a.mean_pct)||Math.abs(Number(a.mean_pct))>meanCap)blocked.push(`${name} ${signalName} ${stat} mean_pct outside cap`);else x.mean_pct=Number(a.mean_pct);}
...
const output={schema_version:'2.4.0',season:2026,week:src.week??null,status:src.status==='SELF_TEST'?'SELF_TEST':(review.length?'REVIEW_REQUIRED':src.status||'NORMALIZED'),generated_at:generatedAt,sportsbook_inputs_used:false,players};
const report={generated_at:generatedAt,result:blocked.length?'BLOCKED':(review.length?'REVIEW_REQUIRED':'PASS'),mode:'SHADOW_ONLY',actionable:false,input_status:src.status,players:Object.keys(players).length,authoritative_live_players:liveByNorm.size,unknown_availability_players:unknownAvailability,current_signals:currentSignals,stale_signals:staleSignals,invalid_signals:invalidSignals,current_role_cohorts:currentRoleCohorts,stale_role_cohorts:staleRoleCohorts,review,blocked,sportsbook_inputs_used:false,safeguards:['Every normalized signal has source and capture time.','Signal freshness is category-specific; injury uses the existing 12-hour limit.','Stale signals are preserved for audit but marked STALE_REVIEW_REQUIRED so projection code can exclude them.','Role cohort evidence is optional, but when supplied it must exist in the historical cohort file and match the player position.','A stale role signal cannot provide a current role-cohort baseline.','Unknown availability remains explicit null/UNKNOWN and is REVIEW_REQUIRED; it is never converted to active.',`Real weekly inputs must match the exact authoritative ${activeCount}-player universe, not merely a nonzero subset.`,'Missing signals remain missing and are not converted to zero or neutral evidence.','Per-signal mean/uncertainty adjustments are bounded by the source contract.']};
fs.writeFileSync(path.join(root,outPath),JSON.stringify(output,null,2)+'\n');fs.writeFileSync(path.join(root,reportPath),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));if(blocked.length)process.exit(1);if(process.argv.includes('--require-current')&&review.length)process.exit(2);