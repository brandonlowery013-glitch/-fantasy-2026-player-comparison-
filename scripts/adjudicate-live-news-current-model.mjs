...
import {directPlayerEvidence,mentionsPlayer,structuredSubjectMatches,signalFields,sourcePlayerFragments} from './live-news-evidence-binding.mjs';
...
export function playerLocalEvidence(player,r,canonical=[]){return directPlayerEvidence(player,r,canonical);}
...
export function connectedOffenseContext(player,r,canonical=[]){
  ...
  for(const s of r?.material_news_signals||[]){
    ...
    for(const qb of teammates){
      ...
      const fragments=sourcePlayerFragments(qb,s,canonical);
      const evidence=lower(fragments.join(' '));
...
export function decisionFor(player,r,rc={},canonical=[]){
  ...
  const t=playerLocalEvidence(player,r,canonical);
  ...
  if(!t){
    ...
    return{status:'ADJUDICATED',horizon:'CURRENT_OUTLOOK',player_effect:'HOLD',near_term_projection:'HOLD',season_long_effect:'HOLD',confidence:'LOW',binding_status:'UNBOUND_MATERIAL_EVIDENCE',reason:'Material source text was present, but no player-specific event fragment, athlete-bound subject, or verified causal connected-player context was found; model/news impact is blocked pending review.',numeric_recalculation_status:rc?.status||'NOT_TRIGGERED',connected_player_effects:[]};
  }
...
function runSelfTest(){
  ...
  const contextual={status:'MATERIAL_CHANGE',material_news_signals:[{headline:'What Tua Tagovailoa being named the starter means for Bijan Robinson',description:'Tua Tagovailoa was named the Falcons starter; what it means for Bijan Robinson and Drake London.'}]};
  const holdTua=decisionFor(atl[0],contextual,{},atl);
  if(holdTua.player_effect!=='HOLD'||holdTua.binding_status!=='CONNECTED_PLAYER_CONTEXT'||holdTua.connected_player_effects?.[0]?.code!=='QB_STARTER_CHANGE_REVIEW')throw new Error('Regression failed: Tua starter context must not become a Bijan role upgrade.');
  const holdLondon=decisionFor(atl[1],contextual,{},atl);
  if(holdLondon.player_effect!=='HOLD'||holdLondon.binding_status!=='CONNECTED_PLAYER_CONTEXT')throw new Error('Regression failed: Tua starter context must not directly upgrade Drake London.');
  ...
  const ind=[{n:'Jonathan Taylor',p:'RB',t:'Indianapolis Colts'},{n:'Sean McKeon',p:'TE',t:'Indianapolis Colts'}];
  const jt=decisionFor(ind[0],{status:'MATERIAL_CHANGE',material_news_signals:[{headline:'Colts signed RB Jonathan Taylor to an extension and placed TE Sean McKeon on injured reserve',description:'Jonathan Taylor signed an extension. Sean McKeon was placed on IR.'}]},{},ind);
  if(jt.player_effect==='DOWNGRADE'||jt.season_long_effect==='DOWNGRADE'||!jt.evidence_excerpt.includes('jonathan taylor')||jt.evidence_excerpt.includes('sean mckeon')||jt.evidence_excerpt.includes('injured reserve'))throw new Error('Regression failed: Sean McKeon IR contaminated Jonathan Taylor.');
  const trueRole=decisionFor(atl[0],{status:'MATERIAL_CHANGE',material_news_signals:[{headline:'Bijan Robinson remains Falcons starting running back',description:'Bijan Robinson will remain the starting running back and is expected to lead the backfield.'}]},{},atl);
  if(trueRole.player_effect!=='UPGRADE')throw new Error('Regression failed: player-specific Bijan role evidence not recognized.');
  const trueIr=decisionFor(ind[0],{status:'MATERIAL_CHANGE',material_news_signals:[{headline:'Colts placed RB Jonathan Taylor on injured reserve',description:'Jonathan Taylor was placed on injured reserve and will miss multiple weeks.'}]},{},ind);
  if(trueIr.player_effect!=='DOWNGRADE'||trueIr.season_long_effect!=='DOWNGRADE')throw new Error('Regression failed: player-specific Jonathan Taylor IR evidence not recognized.');
  const boundPronoun=decisionFor({n:'Example Player',p:'WR',t:'Example'},{status:'MATERIAL_CHANGE',material_news_signals:[{source:'ESPN_PLAYER',player:'Example Player',headline:'Expected back at practice',description:'He is expected back at practice after an ankle injury.'}]},{},[]);
