...
const aliasesFor=name=>{const n=lower(name).trim();const parts=n.split(/\s+/);return [...new Set([n,parts.length>1?parts.at(-1):n].filter(Boolean))];};
const mentionsPlayer=(text,name)=>aliasesFor(name).some(a=>new RegExp(`(^|[^a-z])${esc(a)}([^a-z]|$)`,'i').test(String(text||'')));
const structuredSubjectMatches=(signal,name)=>[signal?.player,signal?.player_name,signal?.subject,signal?.athlete].some(x=>x&&lower(x)===lower(name));
...
export function playerLocalEvidence(player,r){
  ...
  for(const s of r?.material_news_signals||[]){
    ...
    if(structuredSubjectMatches(s,player.n)){pieces.push(...fields);continue;}
    for(const field of fields){for(const clause of splitClauses(field)){if(mentionsPlayer(clause,player.n))pieces.push(clause);}}
  }
...
export function decisionFor(player,r,rc={}){
  ...
  if(!t)return{status:'ADJUDICATED',horizon:'CURRENT_OUTLOOK',player_effect:'HOLD',near_term_projection:'HOLD',season_long_effect:'HOLD',confidence:'LOW',binding_status:'UNBOUND_MATERIAL_EVIDENCE',reason:'Material source text was present, but no player-specific clause or structured subject binding was verified; model impact is blocked pending review.',numeric_recalculation_status:rc?.status||'NOT_TRIGGERED'};
  ...
  return{status:'ADJUDICATED',horizon,player_effect:playerEffect,near_term_projection:nearTermProjection,season_long_effect:seasonLong,confidence,binding_status:'PLAYER_SPECIFIC',reason,numeric_recalculation_status:rc?.status||'NOT_TRIGGERED',proposed_projected_ppr:rc?.proposed_projected_ppr??null,proposed_score:rc?.proposed_score??null,proposed_true_value_rank:rc?.proposed_true_value_rank??null};
}
...
function runSelfTest(){
  ...
  if(holdLondon.player_effect!=='HOLD')throw new Error('Regression failed: unrelated starter text changed Drake London.');
  const jt=decisionFor({n:'Jonathan Taylor'},{status:'MATERIAL_CHANGE',material_news_signals:[{headline:'Colts extend Jonathan Taylor; Sean McKeon placed on injured reserve',description:'Jonathan Taylor signed an extension. Sean McKeon was placed on IR.'}]},{});
  if(jt.player_effect==='DOWNGRADE'||jt.season_long_effect==='DOWNGRADE')throw new Error('Regression failed: another player IR move downgraded Jonathan Taylor.');
  ...
  console.log(JSON.stringify({result:'PASS',tests:5},null,2));
}
...
function connectedEffects(player,r,decision){
  const out=[];for(const c of review.materially_implicated_untracked||[]){const txt=lower([c.reason,...(c.material_news_signals||[]).flatMap(x=>[x.headline,x.description,x.matched_context])].join(' '));if(!mentionsPlayer(txt,player.n)&&!mentionsPlayer(playerLocalEvidence(player,r),c.player))continue;out.push({player:c.player,effect:decision.player_effect==='DOWNGRADE'?'UPGRADE_CONTINGENT_OPPORTUNITY':decision.player_effect==='UPGRADE'?'DOWNGRADE_CONTINGENT_OPPORTUNITY':'REVIEW',reason:c.reason||'Connected-player opportunity changes with the primary player development.'});}return out;
}
...
for(const p of canonical){
  ...
  if(d.binding_status==='UNBOUND_MATERIAL_EVIDENCE'&&d.player_effect!=='HOLD')auditErrors.push(`${p.n}: unbound evidence produced a model move`);
...
write('analysis/live-news-decision-audit-current.json',{schema_version:'1.1.0',generated_at:new Date().toISOString(),authoritative:true,policy:'PLAYER_SPECIFIC_CLAUSE_BINDING_REQUIRED_FOR_MODEL_MOVES; DECIDE_THEN_SELF_AUDIT_CURRENT_OUTLOOK; STATIC_BASELINE_UNTOUCHED; SEASON_LONG_NUMERIC_MOVES_REQUIRE_VALIDATED_QUANTITATIVE_SUPPORT',universe:expected,counts,decisions});
