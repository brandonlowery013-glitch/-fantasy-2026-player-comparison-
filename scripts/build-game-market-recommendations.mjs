import {gameContextEvidence,personnelDecisionGate,teamStateSummary} from '../lib/game-context-evidence.mjs';
...
=======
  const gate=personnelDecisionGate(game.personnel_context,game.home_team,game.away_team);
  const gated=rec=>gate.status==='PASS'?{decision:'PASS',selection:null,confidence:null,reason:gate.reason}:rec;
  games[gameId]={personnel_context:game.personnel_context??null,team_state:teamStateSummary(game.personnel_context,game.home_team,game.away_team),personnel_decision_gate:gate,context_evidence:gameContextEvidence(self?null:footballContext,game,currentWeek),week:mg.week,away_team:game.away_team,home_team:game.home_team,kickoff:mg.kickoff||game.event_start,football_projection:{home_score_mean:game.model.home_score_mean,away_score_mean:game.model.away_score_mean,model_home_spread:game.model.model_home_spread,model_total:game.model.model_total,home_win_probability:game.model.home_win_probability,away_win_probability:game.model.away_win_probability,tie_probability:game.model.tie_probability},snapshot_evaluations:evaluations,current_recommendations:latest?{snapshot_id:latest.snapshot_id,captured_at:latest.captured_at,book:latest.book,spread:gated(latest.markets.spread?.recommendation||{decision:'PASS',selection:null,confidence:null,reason:'Spread market unavailable'}),total:gated(latest.markets.total?.recommendation||{decision:'PASS',selection:null,confidence:null,reason:'Total market unavailable'}),moneyline:gated(latest.markets.moneyline?.recommendation||{decision:'PASS',selection:null,confidence:null,reason:'Moneyline market unavailable'})}:null,sportsbook_inputs_used_for_football_projection:false,mode:'SHADOW_ONLY',actionable:false};
>>>>>>> theirs
}
