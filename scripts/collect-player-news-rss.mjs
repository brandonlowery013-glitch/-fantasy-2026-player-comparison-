...
for(const p of ledger.players||[]){
  const q=`\"${p.player}\" NFL`;
  const url=`https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=en-US&gl=US&ceid=US:en`;
  try{
    const xml=await get(url);fetched++;
    const phrase=norm(p.player);
    const rows=items(xml).filter(x=>{const t=Date.parse(x.pub||'');return Number.isFinite(t)&&t>=START&&norm(`${x.title} ${x.desc}`).includes(phrase);}).slice(0,MAX);
    p.external_news_mentions=p.external_news_mentions||[];
    for(const x of rows){
      ...
      const m={source:'GOOGLE_NEWS_RSS',publisher:x.sourceName,publisher_url:x.sourceUrl||null,source_tier:tier,headline:x.title,description:x.desc,published:new Date(x.pub).toISOString(),url:x.link,context_scope:'DIRECT_PLAYER_EXTERNAL_DISCOVERY',player_bound:true,proposal_only:true};
      if(!p.external_news_mentions.some(y=>y.headline===m.headline&&y.publisher===m.publisher)){p.external_news_mentions.push(m);added++;}
    }
  }catch(e){failures.push(`${p.player}: ${e.message}`);}
  await sleep(20);
}
