...
const triggerRe=/\b(new|first year|new look|overhaul|revamp|install|installation|transition|changed|change|different)\b.{0,80}\b(offense|offensive coordinator|oc|play caller|playcaller|scheme|system|quarterback|qb)\b|\b(new offensive coordinator|new oc|new play caller|new playcaller|new offense|new scheme|new system|quarterback competition|qb competition|named .* starting quarterback|named .* starter|starting quarterback|new starting quarterback)\b/i;
const majorPersonnelRe=/\b(traded|trade|signed|signing|released|departure|departed|added|addition)\b.{0,90}\b(quarterback|qb|wide receiver|receiver|tight end|running back)\b/i;
...
for(const row of report.rows||[]){
  for(const e of row.development_evidence||[]){
    if(!e?.team_context_only)continue;
    const txt=norm(`${e.headline||''} ${e.description||''}`);
    ...
    const arr=triggersByTeam.get(row.team)||[];
    const key=e.url||`${e.headline}|${e.description}`;
    if(!arr.some(x=>(x.url||`${x.headline}|${x.description}`)===key))arr.push({...e,cluster_trigger:true});
    triggersByTeam.set(row.team,arr);
  }
}
const clusterRows=[];
for(const p of players){
  ...
  row.development_evidence=[...(row.development_evidence||[]),...injected].slice(0,28);
