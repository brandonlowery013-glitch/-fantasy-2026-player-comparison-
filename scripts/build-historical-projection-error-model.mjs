...
const authoritativeUniverse=Number(sourceOfTruth.active_player_model);
...
const blocked=[];
if(!Number.isInteger(authoritativeUniverse)||authoritativeUniverse<=0)blocked.push(`invalid authoritative universe: ${sourceOfTruth.active_player_model}`);
if(ref.live_player_universe_count!==authoritativeUniverse)blocked.push(`live universe mismatch: expected ${authoritativeUniverse}, found ${ref.live_player_universe_count}`);
...
console.log(JSON.stringify({result:report.result,authoritative_player_universe:authoritativeUniverse,reference_live_player_universe:ref.live_player_universe_count,tuning_predictions:tuneTotal,holdout_predictions:evalTotal,blocked,projection_error_sd:Object.fromEntries(Object.entries(models).map(([p,s])=>[p,Object.fromEntries(Object.entries(s).map(([st,x])=>[st,x.projection_error_sd]))]))},null,2));
