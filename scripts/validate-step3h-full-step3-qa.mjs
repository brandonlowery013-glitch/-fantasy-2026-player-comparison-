...
const a=read(paths.a),b=read(paths.b),c=read(paths.c),d=read(paths.d),e=read(paths.e),f=read(paths.f),fv=read(paths.fv),bb=read(paths.bb),hs=read(paths.hs),season=read(paths.season),weekly=read(paths.weekly),games=read(paths.games),hist=read(paths.hist),g=read(paths.g),gv=read(paths.gv);
...
add('3C_zero_live_movement',c.live_projection_movement===0&&c.live_rank_movement===0&&c.live_player_files_modified===false,'projection/rank movement zero');
...
add('3D_no_approved_changes',d.approved_change_count===0,`approved=${d.approved_change_count}`);
add('3D_zero_live_movement',d.live_projection_movement===0&&d.live_rank_movement===0,'zero live movement');
...
add('3E_zero_live_movement',e.live_projection_movement===0&&e.live_rank_movement===0&&e.live_player_files_modified===false,'zero live movement');
...
const report={generated_at:new Date().toISOString(),step:'STEP_3H_FULL_STEP3_QA',result:failed.length?'FAIL':'PASS',failed_count:failed.length,checks,step3_completion:failed.length?'NOT_COMPLETE':'COMPLETE_READY_FOR_STEP_4_DATA_INTEGRITY',next_step:failed.length?null:'STEP_4_DATA_INTEGRITY'};
