...
if(totalDepthRows<160)qualityFailures.push(`Depth coverage too low: ${totalDepthRows} rows`);
if(matchedDepthRows<80)qualityFailures.push(`Tracked depth coverage too low: ${matchedDepthRows}/${activeCount}`);
const uniqueTracked=new Set(generated.map(x=>x.player));
if(uniqueTracked.size<80)qualityFailures.push(`Tracked signal coverage too low: ${uniqueTracked.size}/${activeCount}`);
if(!generated.some(x=>x.signal_type==='role'))qualityFailures.push('No role snapshots generated');
if(!generated.some(x=>x.signal_type==='qb_context'))qualityFailures.push('No QB context snapshots generated');
if(endpointFailures.length)console.warn(`Substantive live context endpoint warnings:\n${endpointFailures.join('\n')}`);
if(qualityFailures.length)throw new Error(`Substantive live context collection failed quality gates:\n${qualityFailures.join('\n')}\nEndpoint warnings (${endpointFailures.length}):\n${endpointFailures.join('\n')}`);
...
const report={generated_at:captured,result:'PASS',week,teams:teams.size,total_depth_rows:totalDepthRows,matched_tracked_depth_rows:matchedDepthRows,unique_tracked_players_with_signals:uniqueTracked.size,generated_rows:generated.length,appended_rows:added.length,endpoint_warning_count:endpointFailures.length,endpoint_warnings:endpointFailures,counts:Object.fromEntries(contract.write_contract.allowed_signal_types.map(t=>[t,generated.filter(x=>x.signal_type===t).length])),sportsbook_inputs_used:false};
