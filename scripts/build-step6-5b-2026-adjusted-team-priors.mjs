...
function evidenceByTeam(ledger){
  ...
  for(const row of ledger.evidence||ledger.items||[]){
    ...
    out[team].push({subject:row.subject||null,evidence_type:row.evidence_type||null,status:row.status,affected_engines:row.affected_engines||[],reason:row.reason||null,confidence:row.confidence??null,projection_authority:row.projection_authority??0});
  }
  return out;
}
function build(rows,ledger){
  ...
  const ev=evidenceByTeam(ledger); const teams={};
  for(const t of TEAM_CODES){const a=agg[t];teams[t]={games:a.games,base_2025_points_for_mean:a.games?a.pf/a.games:null,base_2025_points_allowed_mean:a.games?a.pa/a.games:null,league_points_per_team_game_2025:league,verified_2026_adjustment_features:ev[t],numeric_adjustment_authority:0,adjusted_2026_points_for_mean:null,adjusted_2026_points_allowed_mean:null,status:a.games===17?'BASELINE_READY_ADJUSTMENTS_ZERO_AUTHORITY':'BASELINE_INCOMPLETE'};}
  return {schema_version:'STEP6_5B_2026_ADJUSTED_TEAM_PRIORS_1.0.0',season:2026,status:'SHADOW_BASELINE_READY_ADJUSTMENTS_UNCALIBRATED',sportsbook_inputs_used:false,base_season:2025,source:{name:'nflverse schedules',url:SCHEDULE_URL},policy:{formula:'2025 performance baseline + validated 2026 personnel/scheme/availability adjustments = 2026 preseason prior',current_numeric_rule:'Only the 2025 scoring baseline has numeric authority. 2026 personnel/coaching/injury feature rows are attached with zero direct point-value authority until leakage-safe calibration validates coefficients.',double_count_rule:'A canonical football fact may feed multiple engines, but the same effect may not be applied twice through both a team prior adjustment and a direct player adjustment without explicit decomposition.'},league:{points_per_team_game_2025:league},teams};
}
async function main(){
  const ledger=readJson(LEDGER); let text;
  if(process.argv.includes('--self-test')){
    text='season,game_type,home_team,away_team,home_score,away_score\n2025,REG,ARI,ATL,24,20\n2025,REG,ATL,ARI,17,21\n';
    const x=build(csvRows(text),ledger); if(!x.teams.ARI||x.sportsbook_inputs_used!==false||x.teams.ARI.numeric_adjustment_authority!==0)process.exit(1); console.log(JSON.stringify({result:'PASS',mode:'SELF_TEST'})); return;
  }
  ...
  const out=build(csvRows(text),ledger); fs.mkdirSync(path.dirname(path.join(root,OUT)),{recursive:true}); fs.writeFileSync(path.join(root,OUT),JSON.stringify(out,null,2)+'\n');
  ...
  console.log(JSON.stringify({result:'PASS',teams:Object.keys(out.teams).length,league_ppg:out.league.points_per_team_game_2025},null,2));
}
await main();
