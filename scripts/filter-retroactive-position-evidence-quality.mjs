...
function canonicalCategories(e,pos,player){
  ...
  const healthBasis=local.replace(/\b(?:not|non)\s+injury\s+related\b/gi,' ');
  const physicalHealth=strongPhysicalHealth.test(healthBasis);
  ...
  if(healthQualified){out.add('readiness');if(/\b(rehab|recovery|recovering|returning from|coming back from|months removed|year removed|surgery last|acl|achilles|meniscus|lcl|mcl)\b/i.test(healthBasis))out.add('prior_season_injury_recovery');}
