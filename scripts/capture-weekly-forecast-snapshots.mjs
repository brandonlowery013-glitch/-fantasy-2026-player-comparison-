...
export function forecastId({season,week,player,stat,event_start}){return `F-${hash([season,week,player,stat,event_start].join('|'))}`;}
...
export function captureForecasts({distributions,schedule,ledger,nowIso}){
  ...
  for(const [gameId,g] of games){
    const start=parseTime(g.event_start||g.kickoff);if(start==null){blocked.push(`${gameId} missing valid event_start`);continue;}if(g.verified!==true){skipped.push(`${gameId} event start not verified`);continue;}
  ...
  const existing=new Map();for(const row of ledger.forecasts||[]){if(existing.has(row.forecast_id))blocked.push(`duplicate existing forecast_id ${row.forecast_id}`);existing.set(row.forecast_id,row);}
  ...
  for(const [player,p] of Object.entries(distributions.distributions||{})){
    const matches=playerGames.get(player)||[];if(matches.length===0){skipped.push(`${player} missing verified event mapping`);continue;}if(matches.length>1){blocked.push(`${player} mapped to multiple verified games`);continue;}
    const game=matches[0];if(now>=game.start){skipped.push(`${player} kickoff already reached`);continue;}
...
function selfTest(){
  const distributions={season:2026,week:1,generated_at:'2026-09-09T12:00:00Z',distributions:{'Test Player':{position:'WR',distributions:{receiving_yards:{status:'SHADOW_ONLY',family:'normal',mean:71,sd:24,parameters:{mu:71,sigma:24,nested:{a:1,b:2}}},receiving_tds:{status:'REVIEW_REQUIRED'}}}}};
  const schedule={games:{G1:{event_start:'2026-09-10T20:00:00Z',verified:true,players:['Test Player']}}},empty={schema_version:'1.0.0',season:2026,status:'AWAITING_WEEKLY_FORECASTS',forecasts:[]};
