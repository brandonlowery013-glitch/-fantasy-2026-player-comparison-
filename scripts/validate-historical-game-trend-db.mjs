...
if(c.schema_version!=='1.1.0')errors.push('contract schema');
if(c.status!=='STEP_4_HISTORICAL_GAME_CONTEXT_STORE_SHADOW')errors.push('contract status');
if(c.mode!=='DESCRIPTIVE_CONTEXT_ONLY'||c.actionable!==false||c.production_numeric_authority!==0)errors.push('contract authority');
for(const d of requiredDims)if(!c.convenience_summary_dimensions?.includes(d))errors.push(`missing convenience summary ${d}`);
if(c.dimension_policy?.predefined_dimensions_are_convenience_summaries_not_model_features!==true)errors.push('dimension privilege guard');
if(c.dimension_policy?.examples_do_not_define_user_intent!==true)errors.push('example intent guard');
if(c.dimension_policy?.no_dimension_has_intrinsic_predictive_weight!==true)errors.push('intrinsic predictive weight guard');
...
if(!c.guardrails?.some(x=>/predictive weight until separately backtested/i.test(x)))errors.push('optional backtest gate');
...
if(process.argv.includes('--require-output')){
  if(!fs.existsSync(OUT))errors.push('generated output missing');
    ...
    for(const d of requiredDims)if(!o.trend_dimensions?.includes(d))errors.push(`output missing convenience summary ${d}`);
...
console.log(JSON.stringify({result:errors.length?'BLOCKED':'PASS',status:c.status,mode:c.mode,convenience_summaries:requiredDims.length,output_required:process.argv.includes('--require-output'),errors},null,2));
