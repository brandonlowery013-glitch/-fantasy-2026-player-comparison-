...
function fixture(kind){
  const b={schedule:{season:2026,week:1,games:{}},context:{season:2026,week:1,players:{}},market:{provider_credentials_present:false},freeze:{forecasts:{}},games:{games:{}},gameSnapshots:{games:{}},propSnapshots:{snapshots:[]},finals:0,settled:0,football_market_contamination:false};
  if(kind==='PRESEASON_WAITING_FOR_SOURCE')return b;
  b.schedule.games={G1:{}};if(kind==='SOURCE_READY_CONTEXT_MISSING')return b;
  b.context.players={A:{}};if(kind==='SOURCE_READY_PROVIDER_CREDENTIAL_MISSING'){b.freeze.forecasts={F1:{}};b.games.games={G1:{}};return b;}
  b.market.provider_credentials_present=true;b.freeze.forecasts={F1:{}};b.games.games={G1:{}};
  if(kind==='PREGAME_FULLY_READY'){b.gameSnapshots.games={G1:{}};return b;}
  if(kind==='FINAL_WAITING_FOR_SETTLEMENT'){b.gameSnapshots.games={G1:{}};b.finals=1;return b;}
  if(kind==='POSTGAME_SETTLED'){b.gameSnapshots.games={G1:{}};b.finals=1;b.settled=1;return b;}
  if(kind==='ILLEGAL_CHRONOLOGY_BLOCKED'){b.schedule.games={};b.context.players={};b.freeze.forecasts={F1:{}};b.settled=1;return b;}
  return b;
}
...
if(self){
  ...
  const schedule=safe('data/calibration/weekly-event-schedule-2026.json'),context=safe('data/probability/weekly-football-context-raw-2026.json'),market=safe('data/market/live-market-ingestion-status-2026.json'),freeze=safe('data/calibration/weekly-forecast-capture-2026.json'),games=safe('data/probability/generated/weekly-game-projections-2026.json'),gameSnapshots=safe('data/market/weekly-matchup-market-snapshots-2026.json'),propSnapshots=safe('data/market/player-prop-market-snapshots-2026.json'),results=safe('data/calibration/weekly-forecast-results-2026.json');
  const finals=(results?.final_games||results?.finals||[]).length||0,settled=(results?.settlements||results?.results||[]).length||0;
...
if(!protectedDirectPush)blocked.push('Step 24 must not direct-push protected main');
if(!stateBranchPush)blocked.push('Step 24 must push exactly one unique production state branch');
if(!guardedPr)blocked.push('Step 24 production state must persist through a required-check PR');
...
const now=new Date().toISOString(),current=self?null:cases.CURRENT;
const invariants={single_commit_per_cycle:commitCount===1,single_push_per_cycle:stateBranchPush,protected_main_pr_only:protectedDirectPush,guardrail_before_state_merge:guardedPr,shared_concurrency_lock:concurrencyOk,provider_secret_wired:secretWired,exclusive_schedule_ownership_validator:ownershipValidatorPresent};
const out={schema_version:'1.2.0',season:2026,step:25,mode:'SHADOW_ONLY',actionable:false,generated_at:now,overall_status:blocked.length?'BLOCKED':current?.status||'SELF_TEST_PASS',reason:blocked.length?blocked.join('; '):current?.reason||'All dry-run scenarios passed.',provider_secret_name:'ODDS_API_KEY',provider_secret_value_observable:false,production_invariants:invariants,evidence:current?.evidence||null,blocked};
const report={generated_at:now,result:blocked.length?'BLOCKED':'PASS',scenario_statuses:Object.fromEntries(Object.entries(cases).map(([k,v])=>[k,v.status])),production_invariants:invariants,blocked};
