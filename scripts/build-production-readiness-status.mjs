...
function derive(s){
  ...
  const sourceReady=scheduleGames>0;
  ...
  if((gameMarkets>0||propMarkets>0)&&!sourceReady)blocked.push('market snapshots exist without verified source schedule');
  if(frozen>0&&!sourceReady)blocked.push('frozen forecasts exist without verified source schedule');
  ...
  else if(!sourceReady){status='WAITING_FOR_SOURCE';reason='Verified 2026 weekly source events are not populated yet.';}
  else if(!providerPresent){status='WAITING_FOR_PROVIDER_CREDENTIALS';reason='Verified source exists, but ODDS_API_KEY is not confirmed by the live ingestion layer.';}
  else if(!frozen||!gameProjections){status='WAITING_FOR_FORECAST';reason='Source and provider are ready, but required frozen/player or game forecasts are incomplete.';}
  else if(!gameMarkets&&!propMarkets){status='WAITING_FOR_MARKETS';reason='Forecasts are ready, but no captured sportsbook snapshots are available yet.';}
  ...
  return {status,reason,blocked,evidence:{schedule_games:scheduleGames,provider_credentials_present:providerPresent,frozen_forecasts:frozen,game_projections:gameProjections,game_market_games:gameMarkets,player_prop_snapshots:propMarkets,final_games:finals,settled_forecasts:settled}};
}

function fixture(kind){
  const base={schedule:{season:2026,games:{}},market:{provider_credentials_present:false},freeze:{forecasts:{}},games:{games:{}},gameSnapshots:{games:{}},propSnapshots:{snapshots:[]},finals:0,settled:0,football_market_contamination:false};
  if(kind==='PRESEASON_WAITING_FOR_SOURCE')return base;
  base.schedule.games={G1:{}};
  if(kind==='SOURCE_READY_PROVIDER_CREDENTIAL_MISSING')return base;
  ...
  if(kind==='ILLEGAL_CHRONOLOGY_BLOCKED'){base.schedule.games={};base.freeze.forecasts={F1:{}};base.settled=1;return base;}
  return base;
}
...
if(self){
  const expected={PRESEASON_WAITING_FOR_SOURCE:'WAITING_FOR_SOURCE',SOURCE_READY_PROVIDER_CREDENTIAL_MISSING:'WAITING_FOR_PROVIDER_CREDENTIALS',PREGAME_FULLY_READY:'READY',FINAL_WAITING_FOR_SETTLEMENT:'WAITING_FOR_SETTLEMENT',POSTGAME_SETTLED:'READY',ILLEGAL_CHRONOLOGY_BLOCKED:'BLOCKED'};
  ...
  cases.CURRENT=derive({schedule,market,freeze,games,gameSnapshots,propSnapshots,finals,settled,football_market_contamination:games?.sportsbook_inputs_used===true});
}
